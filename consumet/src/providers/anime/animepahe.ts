...
class AnimePahe extends AnimeParser {
   ...
   * sole `…m3u8` match is the playlist; the shared {@link unpackPacker} expands it deterministically
   * (no eval). (The shared {@link Kwik} extractor can't be reused: it is HTTP/1.1 and hard-codes an
  ...
  private unpackKwik = async (embedUrl: string): Promise<string> => {
    ...
    const unpacked = unpackPacker(data, embedUrl);
