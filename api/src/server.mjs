// Self-hosted HTTP API over the anime aggregator, with a Referer-injecting HLS/subtitle
// proxy so streams play in a browser. Internal name: "anime-api".
//
// Env:
...
//   RATE_LIMIT_WATCH       per-IP/min for /watch (extractor + proxy setup), default 30
//   RATE_LIMIT_PROXY       per-IP/min for /proxy (HLS segments — keep HIGH or playback breaks), default 600
...
const RL_TIERS = {
  ...
  proxy: Number(process.env.RATE_LIMIT_PROXY ?? 600),
};
...
app.get('/', async () => {
  ...
  // VM internals (cloakbrowser reachability, TLS-impersonation hosts) only when DEBUG_INFO is set
